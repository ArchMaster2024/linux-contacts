    ...
    <section class="flex justify-center items-center mt-2">
        <article class="container mx-auto p-6">
            ...
            <div class="grid grid-cols-4">
                <form action="{{ route('contacts.update', ['contact' => $contact]) }}" class="col-span-2 col-start-2" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <fieldset class="flex flex-col justify-content-center align-items-center p-2 my-2">
                        ...
                        <figure class="flex flex-col justify-center items-center text-gray-800 dark:text-gray-200 text-xs" id="image">
                            <img class="h-96 object-contain rounded-2xl cursor-pointer" src="{{ asset('storage/'.$contact->photo_path) }}" alt="{{ $contact->name . '-foto' }}">
                            <figcaption>
                    ...
                    <fieldset class="flex flex-col p-2 my-2">
                        ...
                        <input type="text" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1 block w-full" id="name" name="name" placeholder="John Doe" value="{{ $contact->name }}">
                        @error('name')
                            <span class="text-red-700 mt-1.5 text-center italic">
                                {{ $message }}
                            </span>
                    ...
                    <fieldset class="flex flex-col justify-content-center align-items-center p-2 my-2">
                        ...
                        <textarea class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1 block w-full" name="direction" id="direction">{{ $contact->direction }}</textarea>
                        @error('direction')
                            <span class="text-red-700 mt-1.5 text-center italic">
                                {{ $message }}
                            </span>
                    ...
                    <fieldset class="flex flex-col justify-content-center align-items-center p-2 my-2">
                        ...
                        <input type="email" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1 block w-full" name="email" id="email" placeholder="example@gmail.com" value="{{ $contact->email }}">
                        @error('email')
                            <span class="text-red-700 mt-1.5 text-center italic">
                                {{ $message }}
                            </span>
